...
**Vanilla reference binary:** `/usr/local/internal-toolchain/platform010-aarch64/bin/python3.12` — this is the Meta internal-toolchain-built CPython, likely compiled with PGO and LTO optimisations. This is the correct baseline: CinderX must beat the production interpreter, not a debug build. All speedup/regression percentages in this document are measured against this binary. Note: the `python-install` binary in `cinderx_dev/` is a different (unoptimised) build and produces different comparison results.
...
**Run 1 — Spec OFF:** `/tmp/cinderx_benchmark_20260223_235420/` on build-host
**Run 2 — Spec ON:** `/tmp/cinderx_benchmark_20260224_000822/` on build-host
**Run 3 — Inliner OFF sweep:** `/tmp/cinderx_sweep_inliner_off_20260224_013133/` on build-host
...
**Regression concern RESOLVED (testkeeper, 10:30):** Force_compile timing on build-host (same machine as Run 1) with reification fix:
...
- Supervisor: ~~clean rebuild on build-host DONE (10:23)
...
**Machine:** build-host (same as Phase 1 Runs 1–3).
...
**Observation:** supervisor's build-host measurement showed 427ms for richards_full (spec ON + inliner ON), compared to Run 1's 225ms. Initial concern: reification/LICM fix caused a 1.9× regression.
...
2. **Compilation scope (10:30):** force_compile on build-host shows ~34ms all configs. — *Confirms no regression under matched methodology.*
...
5. **Empirical resolution (10:55):** testkeeper's ALL_COMPILABLE timing on build-host — all configs 225–230ms, matching Run 1 exactly. **No regression. The build-host 427ms is environmental (thermal throttling, competing load, or hardware difference).**
...
All functions force-compiled, N_ITER=100000, 5 runs, build-host
...
- build-host environmental slowdown (not code regression)

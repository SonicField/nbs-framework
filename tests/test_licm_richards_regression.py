...
FrameState for the deopt path references a bytecode offset that is invalid
in the preheader context, triggering frame.cpp:163 Abort ("couldn't find
non-inlined frame"). The crash occurs during JIT compilation of
bench_richards_full, specifically when _richards_schedule's main loop
dispatches to polymorphic task types.
...
Pattern:
  ...
  - The hoisted GuardType's FrameState carries the wrong bytecode offset
...
NOTE: Each test is wrapped in its own function to avoid CinderX shadow
bytecode cache pollution between tests sharing attribute names in the
same scope (see cinderx.init() inline cache scoping bug).
"""
...
def _test_polymorphic_scheduler():
    class TaskBase:
...
def _test_linked_list_polymorphic():
    class Node:
...
def _test_state_machine_polymorphic():
    class State:
...
def _test_nested_loop_invariant_type():
    class Accumulator:
        def __init__(self):
            self.total = 0
        def add(self, x):
...
def _test_while_loop_polymorphic():
    class Counter:
        def __init__(self, n):
            self.n = n
        def decrement(self):
...
def _test_gen_with_method_dispatch():
    class Emitter:
...
def _test_loop_with_attr_access():
    class Config:
        def __init__(self, val):
            self.val = val
            self.multiplier = 1
...
def _test_deep_inheritance_loop():
    class Level0:
        def compute(self, x):
            return x
...
def _test_alternating_dispatch():
    class Adder:
        def apply(self, x):
            return x + 1
...
def _test_mutable_scheduler_deopt():
    class MutableTask:
        def __init__(self, n):
            self.n = n
        def run(self):
...
# ---- Test 15: Rapid type alternation in scheduler (deopt stress) ----
def _test_rapid_type_alternation():
    class TypeA:
        def compute(self, x):
            return x + 1
    ...
    # Warmup with TypeA first to trigger JIT compilation
    a, b = TypeA(), TypeB()
    for _ in range(WARMUP):
        rapid_type_alt(a, 20)
    check_jit_compiled(rapid_type_alt, "rapid_type_alt")
    # Now stress with rapid alternation (50 cycles)
...
def _test_single_iter_dispatch():
    class SingleTask:
        def execute(self):
            return 42
...
def _test_large_polymorphic_set():
    classes = []
...
def _test_richards_like_schedule():
    class TaskState:
        RUNNING = 0
        WAITING = 1
        HELD = 2
...
# ---- Test runner ----
_TESTS = [
    ("polymorphic_scheduler", _test_polymorphic_scheduler),
    ("linked_list_polymorphic", _test_linked_list_polymorphic),
    ("state_machine_polymorphic", _test_state_machine_polymorphic),
    ("richards_schedule", _test_richards_schedule),
    ("chain_dispatch", _test_chain_dispatch),
    ("nested_loop_invariant_type", _test_nested_loop_invariant_type),
    ("while_loop_polymorphic", _test_while_loop_polymorphic),
    ("gen_with_method_dispatch", _test_gen_with_method_dispatch),
    ("loop_with_attr_access", _test_loop_with_attr_access),
    ("deep_inheritance_loop", _test_deep_inheritance_loop),
    ("deopt_type_change", _test_deopt_type_change),
    ("deopt_mid_list", _test_deopt_mid_list),
    ("alternating_dispatch", _test_alternating_dispatch),
    ("mutable_scheduler_deopt", _test_mutable_scheduler_deopt),
    ("rapid_type_alternation", _test_rapid_type_alternation),
    ("empty_task_list", _test_empty_task_list),
    ("single_iter_dispatch", _test_single_iter_dispatch),
    ("nested_polymorphic", _test_nested_polymorphic),
    ("large_polymorphic_set", _test_large_polymorphic_set),
    ("richards_like_schedule", _test_richards_like_schedule),
]

...
**Result:** +4.2% speedup on module attribute access. Implementation: ~40 lines across 3 files, built and verified on build-host against Python 3.14.0.
...
Built CinderX from PyPI source tarball (cinderx-2026.2.2.0) on build-host (x86_64):
...
The build-host environment (where our aarch64 changes live) was unreachable from build-host — DNS resolution fails across regions.

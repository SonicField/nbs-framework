...
CinderX adaptive specialisation work on ARM (build-host

## Quick Reference

| What | Command | Where |
|------|---------|-------|
| Run specialisation tests | `for f in tests/test_*.py; do python3 "$f"; done` | build-host |
| Run upstream CinderX tests | `python3 -m pytest cinderx/PythonLib/test_cinderx/` | build-host |
| Run single specialisation test | `python3 tests/test_<opcode>.py` | build-host |
| Run benchmark (JIT vs vanilla) | `bash cinderx_jit_benchmark.sh` | build-host |
| Run specialisation benchmark | `bash benchmark_specialisation.sh` | build-host |
...
There are multiple benchmark scripts on build-host **Not all are equivalent.**
Using the wrong script produces misleading results.

| Script | Purpose | Specialisation? | Warmup | Baseline |
|--------|---------|-----------------|--------|----------|
| `cinderx_jit_benchmark.sh` | **PRIMARY** — CinderX JIT vs vanilla CPython | Yes (`enable_specialized_opcodes(True)`) | 3-phase: specialise → force_compile → thermal | Vanilla internal-toolchain Python 3.12 (`-I`) |
...
```
/usr/local/internal-toolchain/platform010-aarch64/bin/python3.12
```
...
**Symptom**: Correctness test deopt failures on build-host but not locally.
